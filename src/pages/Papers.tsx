...
export const Papers: React.FC = () => {
  ...
  const fetchPapers = () => {
    setLoading(true);
    questionRepository.getPapersList(classId, selectedSubject).then((data) => {
      setPapers(data);
      setLoading(false);
    });
  };
  ...
  return (
    <div className="space-y-4 pb-36 animate-in fade-in duration-300">
      <HeaderBar
        ...
        title={paramSubjectId ? `${selectedSubject}` : 'प्रश्न पत्र (Papers)'}
        ...
        <div className="space-y-1.5">
          <div className="flex items-center gap-2">
            <span className="inline-flex items-center gap-1 text-[11px] font-bold text-blue-700 dark:text-blue-300 bg-blue-50 dark:bg-blue-950/60 px-2.5 py-0.5 rounded-full border border-blue-200/60 dark:border-blue-900">
              <FileText className="w-3 h-3 text-blue-600 dark:text-blue-400" /> {selectedSubject} Papers
            </span>
            <button
              onClick={fetchPapers}
              disabled={loading}
          ...
          <h2 className="text-lg sm:text-xl font-black text-slate-900 dark:text-white">
            {selectedSubject} प्रश्न पत्र
          </h2>
          <p className="text-xs text-slate-500 dark:text-slate-400">
            वर्षवार एवं सेटवार बोर्ड मॉडल पेपर्स अभ्यास के लिए उपलब्ध हैं।
          </p>
        </div>
      ...
      {years.length > 1 && (
        <div className="flex items-center gap-2 overflow-x-auto pb-1 no-scrollbar">
          <span className="text-xs font-bold text-slate-500 dark:text-slate-400 flex items-center gap-1 px-1">
            <Filter className="w-3.5 h-3.5" /> वर्ष:
          </span>
          <button
