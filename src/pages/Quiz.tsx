      ...
      <div className="bg-white dark:bg-slate-900 rounded-2xl sm:rounded-3xl p-5 sm:p-6 space-y-5 border border-slate-200/80 dark:border-slate-800 shadow-xs">
        ...
        <div className="space-y-2.5 pt-1">
          {currentQuestion.options.map((option, idx) => {
            ...
            let optionStyle =
              'bg-white dark:bg-slate-800/70 border-slate-200/90 dark:border-slate-700 text-slate-800 dark:text-slate-100 hover:border-blue-300 hover:bg-slate-50/60 dark:hover:bg-slate-800 shadow-xs';
            let badgeStyle = 'bg-slate-100 dark:bg-slate-700 text-slate-600 dark:text-slate-300 border border-slate-200 dark:border-slate-600';
            ...
            if (isCurrentSubmitted) {
              if (isCorrectAnswer) {
                ...
                optionStyle = 'bg-rose-50 dark:bg-rose-950/50 border-rose-500 text-rose-900 dark:text-rose-100 font-bold shadow-xs';
                badgeStyle = 'bg-rose-500 text-white';
              } else {
                optionStyle = 'opacity-50 bg-slate-50 dark:bg-slate-800/40 border-slate-200 dark:border-slate-700';
              }
            } else if (isSelected) {
              optionStyle = 'bg-blue-50/80 dark:bg-blue-950/60 border-blue-600 text-blue-900 dark:text-blue-100 font-bold shadow-xs ring-1 ring-blue-600/30';
            ...
            return (
              <button
                ...
                className={`w-full p-4 rounded-xl border text-left transition-all duration-150 flex items-center gap-3.5 cursor-pointer active:scale-[0.99] ${optionStyle}`}
              >
                <span className={`w-8 h-8 rounded-xl font-black text-xs flex items-center justify-center shrink-0 transition-colors ${badgeStyle}`}>
